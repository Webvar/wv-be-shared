...
const GraphQLType = schema.getType(name) as GraphQLObjectType;

const baseTypes = createBaseTypes(
  HASURA_WITHOUT_PREFIX,
  Object.values(schema.getTypeMap()).filter(
    (definition) => definition instanceof GraphQLScalarType
